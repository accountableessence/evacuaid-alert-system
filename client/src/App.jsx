...
function App() {
  ...
  // Permanently enforce dark mode on the document body just in case
  useEffect(() => {
  ...
  return (
    <BrowserRouter>
      {/* Locked into pure Dark Mode base colors */}
      <div className="min-h-screen selection:bg-bwai-purple selection:text-white pb-32 transition-colors duration-500 relative overflow-hidden bg-[#060e1d] text-slate-100">
        ...
        {/* Animated Background Blobs emphasizing the Darker Lilac / Steel palette */}
        ...
        <nav className="sticky top-0 z-40 border-b border-t-0 border-x-0 border-white/30 shadow-[0_4px_30px_rgba(123,156,182,0.3)] bg-[#7B9CB6]/90 backdrop-blur-2xl">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex bg-[#2B3B4C]/60 backdrop-blur-md rounded-full p-1 border border-white/20 shadow-inner w-full md:w-auto overflow-x-auto relative z-10 font-mono">
            ...
            <div className="flex items-center gap-4">
               {/* Removed Theme Toggle entirely per user request */}
               ...
               <div className={`hidden md:flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black shadow-inner uppercase font-mono tracking-wider backdrop-blur-md ${connected ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                 <div className={`w-2 h-2 rounded-full ${connected ? 'bg-green-400 shadow-[0_0_10px_rgba(34,197,94,0.6)]' : 'bg-red-400 shadow-[0_0_10px_rgba(239,68,68,0.6)]'}`}></div>
...
function NavLink({ to, children }) {
  ...
  return (
    <Link 
      to={to} 
      className={`px-5 py-2.5 rounded-full transition-all duration-300 flex-1 text-center whitespace-nowrap ${isActive ? 'bg-white text-[#2B3B4C] shadow-md scale-105 border border-white/50 font-black' : 'text-slate-100 hover:text-white hover:bg-white/20 font-bold'}`}
    >
      {children}
    </Link>
  );
